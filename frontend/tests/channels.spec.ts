...
test.describe('Admin Channels Management', () => {
  ...
  test('can open model prices in the accounting currency', async ({ page }) => {
    const firstRow = page.locator('[data-testid="channels-table"] tbody tr').first()
    ...
    await actionsTrigger.click()
    const modelPricesOption = page.getByRole('menu').getByRole('menuitem', {
      name: /模型价格|Model Prices/i,
    })
    if ((await modelPricesOption.count()) === 0) {
      test.skip()
      return
    }

    const modelPricesResponsePromise = page.waitForResponse((response) => {
      return response.url().includes('/admin/graphql') && response.request().postData()?.includes('GetChannelModelPrices') === true
    })
    await modelPricesOption.click()
    const modelPricesResponse = await modelPricesResponsePromise
    expect(modelPricesResponse.ok()).toBeTruthy()
    const modelPricesPayload = await modelPricesResponse.json()
    expect(modelPricesPayload.errors).toBeUndefined()

    const dialog = page.getByRole('dialog')
    await expect(dialog).toBeVisible()
    await expect(dialog).toContainText(/CNY/i)
    await expect(page.getByRole('heading', { name: '500' })).toHaveCount(0)
    expect(pageErrors.filter((message) => message.includes('Invalid currency code'))).toEqual([])
  })
  ...
  test('can configure model mappings in model mapping dialog', async ({ page }) => {
    ...
    const updateResponsePromise = page.waitForResponse((response) => {
      ...
      return response.request().postData()?.includes('UpdateChannel') ?? false
    })
    await saveButton.click()
    const updateResponse = await updateResponsePromise
    const updatePayload = await updateResponse.json()
    expect(updatePayload.errors, JSON.stringify(updatePayload.errors)).toBeUndefined()
    expect(JSON.stringify(updatePayload.data)).toContain(aliasName)

    // Wait for dialog to close
    await expect(settingsDialog).not.toBeVisible({ timeout: 10000 })

    // Force a fresh channel query before reopening. The list cache can still
    // contain the pre-mutation Channel object while parallel E2E workers are
    // generating unrelated channel updates.
    await page.reload({ waitUntil: 'domcontentloaded' })
    await channelsTable.waitFor({ state: 'visible', timeout: 20000 })
...
test.describe('Admin Channels mobile actions', () => {
  test.use({ viewport: { width: 375, height: 812 } })

  test('opens model prices from a channel card overflow menu', async ({ page }) => {
    await gotoAndEnsureAuth(page, '/channels')

    const channelCard = page.getByTestId('mobile-channel-card').first()
    if (!(await channelCard.isVisible({ timeout: 15000 }).catch(() => false))) {
      test.skip(true, 'No channel fixture is available for mobile action coverage')
    }

    const actionsTrigger = channelCard.getByTestId('mobile-channel-actions')
    if (!(await actionsTrigger.isVisible().catch(() => false))) {
      test.skip(true, 'The signed-in user cannot edit channels')
    }

    await actionsTrigger.click()
    const modelPriceItem = page.getByRole('menu').getByRole('menuitem', { name: /模型价格|Model Price/i })
    await expect(modelPriceItem).toBeVisible()
    await modelPriceItem.click()
    ...
    expect(footerBounds!.y + footerBounds!.height).toBeLessThanOrEqual(page.viewportSize()!.height)

    const dialogBody = dialog.getByTestId('model-price-dialog-body')
    await dialogBody.evaluate((body) => {
      body.scrollTop = body.scrollHeight
    })
    await expect.poll(() => dialogBody.evaluate((body) => body.scrollTop)).toBeGreaterThan(0)
    const scrolledFooterBounds = await footer.boundingBox()
    expect(scrolledFooterBounds).not.toBeNull()
    expect(scrolledFooterBounds!.y + scrolledFooterBounds!.height).toBeLessThanOrEqual(page.viewportSize()!.height)
  })
})

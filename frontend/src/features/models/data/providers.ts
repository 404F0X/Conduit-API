...
const DEVELOPERS_URL =
  'https://raw.githubusercontent.com/404F0X/Conduit-API/refs/heads/main/frontend/src/features/models/data/providers.json';
  ...
  return useQuery<ProvidersData>({
    ...
    queryFn: async () => {
      try {
        ...
        // eslint-disable-next-line no-console -- Preserve diagnostics before using bundled provider data.
  ...
  return useQuery<ProvidersData>({
    ...
    queryFn: async () => {
      try {
        ...
        // eslint-disable-next-line no-console -- Preserve diagnostics before using bundled developer data.

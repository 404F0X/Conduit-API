...
import { fetchProductModeOrDefault, resolveProductLandingPath } from '@/features/product-experience';
...
export function useSignIn() {
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      // Owner users keep the global dashboard; project users land in their workspace.
      const productMode = await fetchProductModeOrDefault();
      const redirectPath = resolveProductLandingPath(productMode, data.user.isOwner);
...
export function useSignUp() {
  ...
  return useMutation({
    ...
    onSuccess: async (data) => {
      ...
      const productMode = await fetchProductModeOrDefault();
      router.navigate({ to: resolveProductLandingPath(productMode, data.user.isOwner) });
    },
    onError: (error: unknown) => {
      toast.error(error instanceof Error ? error.message : i18n.t('auth.signUp.error'));
    },
  });
}

...
export function ApiKeyProfilesDialog({ open, onOpenChange, onSubmit, loading = false, initialData }: ApiKeyProfilesDialogProps) {
  ...
  const { data: availableModels, mutate: fetchModels } = useQueryModels();
  const modelCatalog = useMyModelCatalog();
  const availableChannels = useMemo(() => {
    const channels = new Map<string, string>();
    for (const model of modelCatalog.data?.models || []) {
      for (const route of model.routes) channels.set(route.channelID, route.channelName);
    }
    return [...channels].map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name));
  }, [modelCatalog.data]);
  ...
  useEffect(() => {
    if (open && modelPermissions.canRead) {
      fetchModels({
        statusIn: ['enabled'],
        includeMapping: true,
        includePrefix: true,
  ...
  }, [open, modelPermissions.canRead, fetchModels]);
  ...
  const watchedProfiles = form.watch('profiles') ?? defaultValues.profiles;
                  ...
                  <FormItem className='flex items-center space-y-0 gap-x-3'>
                    ...
                    <FormControl>
                      <Select onValueChange={field.onChange} value={field.value}>
                        ...
                        <SelectContent>
                          {Array.from(new Set(profileNames.filter((name) => name.trim() !== ''))).map((profileName) => (
...
function ProfileCard({
  ...
  // Track profile-name changes with a stable primitive. Depending on the
  // watched array directly can recreate the validation callback on render and
  // repeatedly publish the same form error state.
  const profileNamesSignature = JSON.stringify((form.watch('profiles') || []).map((profile) => profile?.name || ''));
  const profileName = form.watch(`profiles.${profileIndex}.name`);
  const selectedChannelIDs = form.watch(`profiles.${profileIndex}.channelIDs`) || [];
  ...
  const checkDuplicate = useCallback(
    (value: string) => {
      const fieldName = `profiles.${profileIndex}.name` as const;
      ...
      const otherProfiles = (form.getValues('profiles') || []).filter((_profile: ApiKeyProfile, idx: number) => idx !== profileIndex);
      const isDuplicate = otherProfiles.some((p: ApiKeyProfile) => p.name && p.name.trim().toLowerCase() === trimmedValue);

      if (isDuplicate) {
        const message = t('apikeys.validation.duplicateProfileName');
        if (currentError?.type !== 'manual' || currentError.message !== message) {
          form.setError(fieldName, { type: 'manual', message });
        }
      } else if (currentError?.type === 'manual') {
        form.clearErrors(fieldName);
      }
    },
    [form, profileIndex, t]
  ...
  }, [debouncedProfileName, checkDuplicate, profileNamesSignature]);
      ...
      {!isCollapsed && (
        <CardContent className='space-y-6'>
          ...
          <div className='border-t pt-6'>
            <FormField
              control={form.control}
              name={`profiles.${profileIndex}.maxConcurrentRequests`}
              render={({ field }) => (
                <FormItem className='mb-5'>
                  <FormLabel>{t('apikeys.profiles.maxConcurrentRequests')}</FormLabel>
                  <FormDescription>{t('apikeys.profiles.maxConcurrentRequestsDescription')}</FormDescription>
                  <FormControl>
                    <Input
                      type='number'
                      min={0}
                      ...
                      placeholder='0'
                      onChange={(event) =>
                        field.onChange(event.target.value === '' ? null : Math.max(0, Number.parseInt(event.target.value, 10) || 0))
                      }
                    />
                  </FormControl>
                  <FormMessage />
              ...
              <div className='grid gap-3 sm:grid-cols-2'>
                {(['validFrom', 'validUntil'] as const).map((key) => (
                  <FormField
                    key={key}
                    control={form.control}
                    name={`profiles.${profileIndex}.${key}`}
                    render={({ field }) => (
                      <FormItem>
          ...
          <div className='space-y-2'>
            <div className='text-sm font-medium'>{t('apikeys.access.channelsTitle')}</div>
            <p className='text-muted-foreground text-xs'>{t('apikeys.access.channelsDescription')}</p>
            <div className='grid max-h-40 gap-1 overflow-y-auto rounded-md border p-2 sm:grid-cols-2'>
              {availableChannels.map((channel) => {
                const channelID = extractNumberIDAsNumber(channel.id);
                const checked = selectedChannelIDs.includes(channelID);
                return (
                  <label key={channel.id} className='hover:bg-muted flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm'>
                    <Checkbox
                      checked={checked}
                      onCheckedChange={(value) => {
                        const next = value ? [...selectedChannelIDs, channelID] : selectedChannelIDs.filter((id) => id !== channelID);
                        form.setValue(`profiles.${profileIndex}.channelIDs`, next, { shouldDirty: true });
                      }}
                    />
              ...
              {!availableChannels.length && <span className='text-muted-foreground p-2 text-xs'>—</span>}
            </div>
            <p className='text-muted-foreground text-xs'>{t('apikeys.access.allChannelsHint')}</p>
          </div>
        </CardContent>
      )}
    </Card>

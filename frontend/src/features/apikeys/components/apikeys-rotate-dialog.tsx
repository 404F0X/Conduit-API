    ...
    <Dialog open={isDialogOpen.rotate} onOpenChange={handleClose}>
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader>
          ...
          <DialogDescription>{t('apikeys.dialogs.rotate.description', { name: selectedApiKey?.name })}</DialogDescription>
        </DialogHeader>

        <Alert className='border-orange-200 bg-orange-50 dark:border-orange-800 dark:bg-orange-950'>
          <AlertTriangle className='h-4 w-4 text-orange-600 dark:text-orange-400' />
          <AlertDescription className='text-orange-800 dark:text-orange-200'>{t('apikeys.dialogs.rotate.warning')}</AlertDescription>
        </Alert>
        ...
        <DialogFooter className='gap-2'>
          ...
          {!newKey && (
            <Button onClick={handleRotate} disabled={rotateApiKey.isPending}>
              {rotateApiKey.isPending ? <RefreshCw className='mr-2 h-4 w-4 animate-spin' /> : <RefreshCw className='mr-2 h-4 w-4' />}

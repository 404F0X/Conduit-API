...
export type ApiKeyAssignableGroup = {
  id: string;
  name: string;
...
const API_KEY_ASSIGNABLE_GROUPS_QUERY = `
  query ApiKeyAssignableGroups {
    apiKeyAssignableGroups {
      id name description status allowedModelIDs allowedChannelIDs
    }
  }
`;

export function useApiKeyAssignableGroups() {
  const selectedProjectId = useSelectedProjectId();
  return useQuery({
    queryKey: ['api-key-assignable-groups', selectedProjectId],
    queryFn: async () => {
      const headers = selectedProjectId ? { 'X-Project-ID': selectedProjectId } : undefined;
      const data = await graphqlRequest<{ apiKeyAssignableGroups: ApiKeyAssignableGroup[] }>(
        API_KEY_ASSIGNABLE_GROUPS_QUERY,
        undefined,
        headers
      );
      return data.apiKeyAssignableGroups;
    },

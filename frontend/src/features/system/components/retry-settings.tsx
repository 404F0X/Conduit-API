  ...
  useEffect(() => {
    if (retryPolicy) {
      setFormData({
        ...
        costScoreWeight: retryPolicy.costScoreWeight,
        ...
        <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          {formData.enabled && (
            <div className='space-y-4'>
              ...
              <div className='space-y-2'>
                <Label htmlFor='cost-score-weight'>{t('system.retry.costScoreWeight.label')}</Label>
                <div className='text-muted-foreground mb-2 text-sm'>{t('system.retry.costScoreWeight.description')}</div>
                <div className='flex items-center gap-2'>
                  <Input
                    id='cost-score-weight'
                    type='number'
                    min='0'
                    max='100'
                    value={formData.costScoreWeight}
                    onChange={(event) =>
                      handleInputChange('costScoreWeight', Math.min(100, Math.max(0, parseInt(event.target.value) || 0)))
                    }
                    className='w-32'
                  />

...
const CONTENT_CLEANUP_DEFAULTS: CleanupOption[] = [
  { resourceType: 'request_headers', enabled: false, cleanupDays: 30 },
  { resourceType: 'request_bodies', enabled: false, cleanupDays: 30 },
  { resourceType: 'response_bodies', enabled: false, cleanupDays: 30 },
  { resourceType: 'response_chunks', enabled: false, cleanupDays: 7 },
];

function withContentCleanupOptions(options: CleanupOption[]): CleanupOption[] {
  const existing = new Set(options.map((option) => option.resourceType));
  return [...options, ...CONTENT_CLEANUP_DEFAULTS.filter((option) => !existing.has(option.resourceType))];
}
...
export function StoragePolicySettings() {
  ...
  const [storagePolicyState, setStoragePolicyState] = useState({
    ...
    storeRequestHeaders: storagePolicy?.storeRequestHeaders ?? true,
    storeRequestBody: storagePolicy?.storeRequestBody ?? true,
    storeResponseBody: storagePolicy?.storeResponseBody ?? true,
    cleanupOptions: withContentCleanupOptions(storagePolicy?.cleanupOptions ?? []),
  });
  ...
  useEffect(() => {
    if (storagePolicy) {
      setStoragePolicyState({
        ...
        storeRequestHeaders: storagePolicy.storeRequestHeaders,
  ...
  const handleSave = async () => {
    ...
    try {
      await updateStoragePolicy.mutateAsync({
        ...
        storeRequestHeaders: storagePolicyState.storeRequestHeaders,
  ...
  const hasChanges =
    ...
    (storagePolicy.storeChunks !== storagePolicyState.storeChunks ||
      ...
      storagePolicy.storeRequestHeaders !== storagePolicyState.storeRequestHeaders ||
      ...
      <Card>
        ...
        <CardContent className='space-y-6'>
          ...
          <div className='flex items-center justify-between'>
            <div className='space-y-0.5'>
              <Label htmlFor='storage-policy-store-request-headers'>{t('system.storage.policy.storeRequestHeaders.label')}</Label>
              <div className='text-muted-foreground text-sm'>{t('system.storage.policy.storeRequestHeaders.description')}</div>
            </div>
            <Switch
              id='storage-policy-store-request-headers'
              checked={storagePolicyState.storeRequestHeaders}
              onCheckedChange={(checked) => setStoragePolicyState({ ...storagePolicyState, storeRequestHeaders: checked })}
              disabled={isLoading}
            />
          </div>

...
export interface SystemGeneralSettings {
  ...
  accountingCurrencyLocked: boolean;
  timezone: string;
  creditDisplayName: string;
  creditsPerAccountingUnit: string;
  exchangeRates: Array<{ currencyCode: string; quotePerAccountingUnit: string }>;
  accountingRateVersion: number;
}
...
const PUBLIC_CHANNEL_HEALTH_SETTINGS_QUERY = `
  query PublicChannelHealthSettings {
    publicChannelHealthSettings { enabled }
  }
`;

const UPDATE_PUBLIC_CHANNEL_HEALTH_SETTINGS_MUTATION = `
  mutation UpdatePublicChannelHealthSettings($input: UpdatePublicChannelHealthSettingsInput!) {
    updatePublicChannelHealthSettings(input: $input)
  }
`;
...
const SYSTEM_GENERAL_SETTINGS_QUERY = `
  query SystemGeneralSettings {
    systemGeneralSettings {
      ...
      accountingCurrencyLocked
      timezone
      creditDisplayName
      creditsPerAccountingUnit
      exchangeRates { currencyCode quotePerAccountingUnit }
      accountingRateVersion
    }
  }
`;
...
export function usePublicChannelHealthSettings() {
  return useQuery({
    queryKey: ['publicChannelHealthSettings'],
    queryFn: async () => {
      const data = await graphqlRequest<{ publicChannelHealthSettings: { enabled: boolean } }>(PUBLIC_CHANNEL_HEALTH_SETTINGS_QUERY);
      return data.publicChannelHealthSettings;
    },
  });
}

export function useUpdatePublicChannelHealthSettings() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (enabled: boolean) => {
      const data = await graphqlRequest<{ updatePublicChannelHealthSettings: boolean }>(UPDATE_PUBLIC_CHANNEL_HEALTH_SETTINGS_MUTATION, {
        input: { enabled },
      });
      return data.updatePublicChannelHealthSettings;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['publicChannelHealthSettings'] });
      queryClient.invalidateQueries({ queryKey: ['publicChannelHealth'] });
      toast.success(i18n.t('common.success.systemUpdated'));
    },
    onError: () => toast.error(i18n.t('common.errors.systemUpdateFailed')),
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      if (data.success && data.data) {
        // Go and both Rust database backends expose the archive as standard
        // base64 in GraphQL. Decode it before saving so the downloaded JSON
        // can be uploaded to the restore mutation without another transform.
        ...
        a.download = `conduit-backup-${timestamp}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

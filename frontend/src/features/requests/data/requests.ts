...
export function useRequest(
  ...
  return useQuery({
    ...
    queryFn: async ({ queryKey: activeQueryKey }) => {
      try {
        const headers = projectId ? { 'X-Project-ID': projectId } : undefined;
        const previousRequest = queryClient.getQueryData<Request>(activeQueryKey);
...
export interface RequestRouteExplanation {
  ...
  finalChannelID?: string | null;
  finalModelID?: string | null;
  terminalError?: string | null;
  affinityKeyClass?: string | null;
  affinityDecision?: string | null;
  retryReasons: Array<{
    sequence: number;
...
export function useRequestRouteExplanation(requestID: string, options?: { projectId?: string | null }) {
  const selectedProjectId = useSelectedProjectId();
  const projectId = options?.projectId !== undefined ? options.projectId : selectedProjectId;
  return useQuery({
    queryKey: ['request-route-explanation', requestID, projectId],
    queryFn: async () => {
      const query = `
        query GetRequestRouteExplanation($requestID: ID!) {
          requestRouteExplanation(requestID: $requestID) {
            requestID projectID requestedModel loadBalanceStrategy
            selectedCandidates rejectedCandidates orderedCandidates
            finalChannelID finalModelID terminalError affinityKeyClass affinityDecision
            retryReasons { sequence channelID status reason responseStatusCode }
          }
        }
      `;
      const headers = projectId ? { 'X-Project-ID': projectId } : undefined;
      const data = await graphqlRequest<{ requestRouteExplanation: RequestRouteExplanation | null }>(query, { requestID }, headers);
      return data.requestRouteExplanation;
    },
    enabled: !!requestID,
  });
}

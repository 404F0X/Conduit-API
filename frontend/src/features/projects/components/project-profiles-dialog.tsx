    ...
    if (initialData && initialData.profiles.length > 0) {
      form.reset({
        ...
        profiles: initialData.profiles.map((p) => ({
          ...
          channelTagsMatchMode: p.channelTagsMatchMode ?? null,
        })),
      });
    } else {
                ...
                <form onSubmit={form.handleSubmit(handleSubmit)} className='space-y-6 px-4'>
                  <div className='space-y-4'>
                    {fields.map((field, profileIndex) => {
                      const profile = watchedProfiles[profileIndex];
                      const hasLegacyChannelSelectors =
                        (profile?.channelIDs?.length ?? 0) > 0 ||
                        (profile?.channelTags?.length ?? 0) > 0 ||
                        profile?.channelTagsMatchMode != null;
                          ...
                          <CardContent className='space-y-6'>
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  ...
                                  <FormControl>
                                    <Input
                                      {...field}
                                      disabled={hasLegacyChannelSelectors}
                                      placeholder={t('projects.profiles.profileNamePlaceholder')}
                                    />
                                  </FormControl>
                                  <FormMessage />
                                </FormItem>
                            ...
                            {hasLegacyChannelSelectors && (
                              <div className='rounded-md border border-amber-500/30 bg-amber-500/5 p-3'>
                                <h4 className='text-sm font-medium'>{t('projects.profiles.legacyChannelsTitle')}</h4>
                                <p className='text-muted-foreground mt-1 text-xs'>{t('projects.profiles.legacyChannelsDescription')}</p>
                                <div className='mt-2 flex flex-wrap gap-1.5'>
                                  {profile.channelIDs?.map((id) => (
                                    <span key={`channel-${id}`} className='bg-background rounded border px-2 py-1 font-mono text-[11px]'>
                                      Channel #{id}
                                    </span>
                                  ))}
                                  {profile.channelTags?.map((tag) => (
                                    <span key={`tag-${tag}`} className='bg-background rounded border px-2 py-1 font-mono text-[11px]'>
                                      #{tag}
                                    </span>
                                  ))}
                                </div>

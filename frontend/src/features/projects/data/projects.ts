...
import {
  ...
  projectSchema,
  type UpdateProjectProfilesInput,
} from './schema';
...
const MY_PRIMARY_PROJECT_QUERY = `
  query MyPrimaryProject {
    myPrimaryProject {
      status
      projectID
      candidateProjectIDs
...
export interface PrimaryProjectResolution {
    ...
    mutationFn: async ({ ids, status }: { ids: string[]; status: 'active' | 'archived' }) => {
      try {
        return await Promise.all(
          ids.map(async (id) => {
            const data = await graphqlRequest<{ updateProjectStatus: Project }>(UPDATE_PROJECT_STATUS_MUTATION, { id, status });
            return projectSchema.parse(data.updateProjectStatus);
          })
        );
      } catch (error) {
        handleError(error, i18n.t(status === 'archived' ? 'projects.errors.bulkArchiveFailed' : 'projects.errors.bulkActivateFailed'));
        throw error;
      }
    },
    onSuccess: (projects, { status }) => {
      toast.success(
        i18n.t(status === 'archived' ? 'projects.messages.bulkArchiveSuccess' : 'projects.messages.bulkActivateSuccess', {
...
export function useMyPrimaryProject(enabled = true) {
  return useQuery({
    queryKey: ['myPrimaryProject'],
    queryFn: async () => {
      const data = await graphqlRequest<{ myPrimaryProject: PrimaryProjectResolution }>(MY_PRIMARY_PROJECT_QUERY);
      return data.myPrimaryProject;
    },
    enabled,
  });
}

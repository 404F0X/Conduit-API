...
export const channelPoliciesSchema = z.object({
  // Rust exposes `Option<CapabilityPolicy>` as a nullable GraphQL enum. Keep
  // the form-facing shape optional while accepting the explicit `null` sent
  // by GraphQL for legacy/unconfigured channels.
  stream: capabilityPolicySchema
    .nullable()
    .optional()
    .transform((policy) => policy ?? undefined),
});
...
export const autoModelMappingRuleSchema = z.object({
  pattern: z.string().min(1),
  publicModelIdTemplate: z.string().min(1),
  createDraft: z.boolean(),
  developerTemplate: z.string().optional().nullable(),
  nameTemplate: z.string().optional().nullable(),
  groupTemplate: z.string().optional().nullable(),
  modelType: z.enum(['chat', 'embedding', 'rerank', 'image_generation', 'video_generation']),
});
export type AutoModelMappingRule = z.infer<typeof autoModelMappingRuleSchema>;

export const errorResponseRewriteRuleSchema = z.object({
  statusCodes: z.array(z.number().int().min(400).max(599)),
  bodyPattern: z.string().optional().nullable(),
  httpStatus: z.number().int().min(400).max(599).optional().nullable(),
  message: z.string().optional().nullable(),
  errorType: z.string().optional().nullable(),
  code: z.string().optional().nullable(),
  body: z.unknown().optional().nullable(),
});
export type ErrorResponseRewriteRule = z.infer<typeof errorResponseRewriteRuleSchema>;

// Channel Settings
export const channelSettingsSchema = z.object({
  managementAdapter: z.enum(['new_api']).optional().nullable(),
  billingCurrency: z
    .string()
    .trim()
    .regex(/^[A-Za-z]{3}$/)
  ...
  autoModelMappingRules: z.array(autoModelMappingRuleSchema).optional().nullable(),
  errorResponseRewriteRules: z.array(errorResponseRewriteRuleSchema).optional().nullable(),
});
...
export const providerQuotaStatusSchema = z.object({
  status: z.string(),
  quotaData: z.unknown().optional().nullable(),
  updatedAt: z.string().optional().nullable(),
  probeAdapter: z.string().optional().nullable(),
  probeVerifiedAt: z.string().optional().nullable(),
});
export type ProviderQuotaStatus = z.infer<typeof providerQuotaStatusSchema>;

export const channelOperationalIssueSchema = z.object({
  category: z.enum(['auth', 'quota', 'rate_limit', 'model', 'unreachable', 'upstream', 'protocol', 'unknown']),
  severity: z.enum(['warning', 'error', 'critical']),
  code: z.string(),
  source: z.string(),
});
export type ChannelOperationalIssue = z.infer<typeof channelOperationalIssueSchema>;

export const channelQuotaProbeResultSchema = z.object({
  success: z.boolean(),
  ...
  unlimited: z.boolean(),
  unlimitedKeyCount: z.number().int().nonnegative(),
  keyCount: z.number().int().nonnegative(),
  requiresPat: z.boolean(),
  balanceSource: z.enum(['key', 'account']).optional().nullable(),
  verified: z.boolean(),
  verifiedAt: z.string().optional().nullable(),
});
export type ChannelQuotaProbeResult = z.infer<typeof channelQuotaProbeResultSchema>;

export const newApiModelPricingProbeSchema = z.object({
  modelId: z.string(),
  billingKind: z.enum(['token', 'per_request', 'tiered_expr', 'unknown', 'unsupported']),
  quality: z.enum(['exact', 'estimated', 'unavailable', 'unsupported']),
  groupRatio: z.string().optional().nullable(),
  inputPerMillion: z.string().optional().nullable(),
  outputPerMillion: z.string().optional().nullable(),
  cacheReadPerMillion: z.string().optional().nullable(),
  cacheWritePerMillion: z.string().optional().nullable(),
  flatPerRequest: z.string().optional().nullable(),
  reason: z.string().optional().nullable(),
});
export type NewApiModelPricingProbe = z.infer<typeof newApiModelPricingProbeSchema>;

export const newApiPricingProbeResultSchema = z.object({
  source: z.string(),
  sourceEndpoint: z.string(),
  fetchedAt: z.string(),
  pricingVersion: z.string().optional().nullable(),
  accountGroup: z.string().optional().nullable(),
  effectiveGroups: z.array(z.string()),
  keyCount: z.number().int().nonnegative(),
  matchedKeyCount: z.number().int().nonnegative(),
  warnings: z.array(z.string()),
  models: z.array(newApiModelPricingProbeSchema),
});
export type NewApiPricingProbeResult = z.infer<typeof newApiPricingProbeResultSchema>;
...
// Channel
const graphqlBaseURLSchema = z
  .string()
  .nullable()
  .transform((baseURL) => baseURL ?? '');
...
export const channelSchema = z.object({
  ...
  // `Channel.baseURL` is nullable in the Rust GraphQL schema. The UI has long
  // treated a missing URL as an empty field, so normalize only at this API
  // boundary instead of widening every channel consumer to `string | null`.
  baseURL: graphqlBaseURLSchema,
  websiteURL: z.string().optional().nullable(),
  quotaCurrency: z.string().default('USD'),
  actualQuotaUsed: z.string().optional().nullable(),
  quotaRemaining: z.string().optional().nullable(),
  name: z.string(),
  ...
  operationalIssue: channelOperationalIssueSchema.optional().nullable(),
});
...
export const createChannelInputSchema = z
  .object({
    ...
    websiteURL: z
      .union([z.literal(''), z.url('channels.dialogs.fields.websiteURL.errors.invalid')])
      .refine((value) => value === '' || /^https?:\/\//i.test(value), 'channels.dialogs.fields.websiteURL.errors.invalid')
      .optional(),
    quotaCurrency: z
      .string()
      .trim()
      .regex(/^[A-Za-z]{3,8}$/, 'channels.dialogs.fields.quotaCurrency.errors.invalid')
      .default('USD'),
    actualQuotaUsed: z
      .string()
      .trim()
      .regex(/^\d+(?:\.\d+)?$/, 'channels.dialogs.fields.quotaAmount.errors.invalid')
      .or(z.literal(''))
      .optional(),
    quotaRemaining: z
      .string()
...
export const updateChannelInputSchema = z
  .object({
    ...
    websiteURL: z
      ...
      .refine((value) => value === '' || /^https?:\/\//i.test(value), 'channels.dialogs.fields.websiteURL.errors.invalid')
      .optional(),
    clearWebsiteUrl: z.boolean().optional(),
    quotaCurrency: z
      .string()
      .trim()
      .regex(/^[A-Za-z]{3,8}$/, 'channels.dialogs.fields.quotaCurrency.errors.invalid')
    ...
    actualQuotaUsed: z
      ...
      .or(z.literal(''))
      .optional(),
    clearActualQuotaUsed: z.boolean().optional(),
    quotaRemaining: z
      .string()
      .trim()
      .regex(/^\d+(?:\.\d+)?$/, 'channels.dialogs.fields.quotaAmount.errors.invalid')
      .or(z.literal(''))
      .optional(),
    clearQuotaRemaining: z.boolean().optional(),

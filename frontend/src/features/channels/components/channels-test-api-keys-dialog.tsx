...
export function ChannelsTestAPIKeysDialog({ open, onOpenChange }: ChannelsTestAPIKeysDialogProps) {
  ...
  const disabledKeySet = useMemo(() => new Set(currentRow?.disabledAPIKeys?.map((item) => item.key) ?? []), [currentRow?.disabledAPIKeys]);
  ...
  const renderKeyCell = (key: string, result?: TestAPIKeyResult) => (
    <TableCell className='font-medium'>
      <div className='flex items-center gap-2'>
        {isTesting && testingKey === key && <IconLoader2 className='text-muted-foreground h-3 w-3 animate-spin' />}
        <code className='bg-muted rounded px-2 py-0.5 font-mono text-sm'>{result ? result.keyPrefix : maskAPIKey(key)}</code>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='flex max-h-[90vh] flex-col sm:max-w-3xl'>
        ...
        <div className='min-h-0 flex-1 space-y-4'>
          {allKeys.length > 0 && (
            <div className='bg-muted/40 flex items-center justify-between gap-4 rounded-md border px-4 py-3'>
            ...
            <ScrollArea className='h-[420px]'>
              <Table>
                ...
                <TableBody>
                  {allKeys.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className='text-muted-foreground h-32 text-center text-sm'>
                    ...
                    allKeys.map((key, index) => (
                      <TableRow key={index}>
                        <TableCell>
                          <Checkbox checked={selectedKeys.has(key)} onCheckedChange={(checked) => handleToggle(key, checked === true)} />
                        </TableCell>
                        {renderKeyCell(key)}
                ...
                <Popover open={confirmDeleteFailed} onOpenChange={setConfirmDeleteFailed}>
                  ...
                  <PopoverContent className='w-80'>
                    <div className='flex flex-col gap-3'>
                      <p className='text-sm'>{t('channels.dialogs.testAPIKeys.confirmDeleteFailed', { count: selectedKeys.size })}</p>

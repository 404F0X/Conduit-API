...
export function ChannelsTable({
  ...
  const { t } = useTranslation();
  const { setSelectedChannels, setResetRowSelection, setOpen, setCurrentRow } = useChannels();
    ...
    <div className='flex min-h-0 min-w-0 flex-1 flex-col'>
      ...
      {Boolean(queryError) && (
        <Alert variant='destructive' className='mt-4 shrink-0'>
          <IconAlertTriangle />
          <AlertTitle>{t('channels.queryError.title')}</AlertTitle>
          <AlertDescription>
            <p>{t('channels.queryError.description')}</p>
            {onRetry && (
              <Button size='sm' variant='outline' onClick={onRetry} disabled={retrying} className='mt-2'>
                <IconRefresh className={retrying ? 'animate-spin' : ''} />
                {retrying ? t('channels.queryError.retrying') : t('common.buttons.retry')}
              </Button>
            )}
          </AlertDescription>
        </Alert>
      ...
      <div className='mt-4 space-y-3 pb-2 lg:hidden'>
        {loading ? (
          Array.from({ length: Math.min(pageSize, 4) }, (_, index) => (
            <div key={index} className='bg-muted/40 h-36 animate-pulse rounded-xl border' />
          ))
        ) : data.length > 0 ? (
          data.map((channel) => (
            <article key={channel.id} data-testid='mobile-channel-card' className='bg-card rounded-xl border p-4 shadow-sm'>
              <div className='flex items-start justify-between gap-3'>
                ...
                <span
                  className={cn(
                    ...
                    channel.status === 'enabled' ? 'bg-emerald-500' : channel.status === 'disabled' ? 'bg-amber-500' : 'bg-slate-400'
                  )}
                  aria-label={t(`channels.status.${channel.status}`)}
                />
              </div>
              <div className='mt-3 flex flex-wrap gap-1.5'>
                <Badge variant='outline'>{t(`channels.types.${channel.type}`, { defaultValue: channel.type })}</Badge>
                <Badge variant='secondary'>{t('channels.mobile.modelCount', { count: channel.supportedModels.length })}</Badge>
                <ChannelManagementAdapterBadge managementAdapter={channel.settings?.managementAdapter} />
                {channel.operationalIssue && (
                  <Badge variant='destructive'>{t(`channels.operations.failureKinds.${channel.operationalIssue.category}`)}</Badge>
                )}
              </div>
              <div className='mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3'>
                ...
                {canWrite && <ChannelOverflowMenu channel={channel} showLabel testId='mobile-channel-actions' />}
              </div>
            </article>
          ))
        ) : !queryError ? (
          <div className='rounded-xl border border-dashed text-center'>
            <ChannelsEmptyState />
          </div>
        ) : null}
      </div>

      <div className='shadow-soft [container-type:inline-size] relative mt-4 hidden w-full min-w-0 flex-1 overflow-auto rounded-2xl border border-[var(--table-border)] lg:flex'>
        <div className='w-full min-w-[64rem]'>
          <Table
            data-testid='channels-table'
            className='min-w-full border-separate border-spacing-0 rounded-2xl bg-[var(--table-background)]'
          >
              ...
              {loading ? (
                ...
                <TableRow className='!bg-[var(--table-background)]'>
                  <TableCell
                    ...
                    className='h-28 w-full !bg-[var(--table-background)] p-0 text-center whitespace-normal'
                  >
                    <ChannelsEmptyState />
                  </TableCell>
                </TableRow>

...
import { Plus, X, Lightbulb, WandSparkles } from 'lucide-react';
...
import {
  type ChannelModelMappingPreview,
  useApplyChannelModelMappings,
  useChannelModelMappingAutomationSettings,
  usePreviewChannelModelMappings,
  useSetChannelModelMappingAutomation,
  useUpdateChannel,
} from '../data/channels';
...
export function ChannelsModelMappingDialog({ open, onOpenChange, currentRow }: Props) {
  ...
  const previewMappings = usePreviewChannelModelMappings();
  const applyMappings = useApplyChannelModelMappings();
  const automationSettings = useChannelModelMappingAutomationSettings(open);
  const setAutomation = useSetChannelModelMappingAutomation();
  ...
  const [mappingPreview, setMappingPreview] = useState<ChannelModelMappingPreview | null>(null);
  ...
  const handlePreviewMappings = async () => {
    try {
      setReplaceConflicts(false);
      setMappingPreview(await previewMappings.mutateAsync(currentRow.id));
    } catch (error) {
  ...
  const handleAutomationToggle = async (enabled: boolean) => {
    try {
      await setAutomation.mutateAsync(enabled);
      if (!enabled) {
        setMappingPreview(null);
        setReplaceConflicts(false);
      }
      toast.success(
        t(
          enabled ? 'channels.dialogs.settings.modelMapping.automationEnabled' : 'channels.dialogs.settings.modelMapping.automationDisabled'
        )
      );
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('common.errors.internalServerError'));
  ...
  const handleApplyMappings = async () => {
    ...
    try {
      await applyMappings.mutateAsync({
        channelID: currentRow.id,
        expectedVersion: mappingPreview.expectedVersion,
        replaceConflicts,
      });
      toast.success(
        t('channels.dialogs.settings.modelMapping.autoApplySuccess', {
          count: mappingPreview.createCount + (replaceConflicts ? mappingPreview.conflictCount : 0),
        })
      );
      onOpenChange(false);
    } catch (error) {
                ...
                <Card>
                  ...
                  <CardContent className='space-y-4'>
                    <div className='bg-muted/30 rounded-lg border p-4'>
                      <div className='bg-background mb-4 flex items-start justify-between gap-4 rounded-md border p-3'>
                        <div>
                          <div className='font-medium'>{t('channels.dialogs.settings.modelMapping.automationSwitch')}</div>
                          <p className='text-muted-foreground mt-1 text-sm'>
                            {t('channels.dialogs.settings.modelMapping.automationSwitchDescription')}
                          </p>
                        </div>
                        <Checkbox
                          checked={automationSettings.data?.enabled ?? false}
                          disabled={automationSettings.isLoading || setAutomation.isPending}
                          onCheckedChange={(checked) => handleAutomationToggle(checked === true)}
                          aria-label={t('channels.dialogs.settings.modelMapping.automationSwitch')}
                        />
                      </div>
                      <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                        <div>
                          <div className='font-medium'>{t('channels.dialogs.settings.modelMapping.autoTitle')}</div>
                          <p className='text-muted-foreground mt-1 text-sm'>
                            {t('channels.dialogs.settings.modelMapping.autoDescription')}
                          </p>
                        </div>
                        <Button
                          type='button'
                          ...
                          disabled={
                            !automationSettings.data?.enabled ||
                            previewMappings.isPending ||
                            applyMappings.isPending ||
                            setAutomation.isPending
                          }
                        >
                          <WandSparkles className='mr-2 h-4 w-4' />
                          {previewMappings.isPending
                            ? t('channels.dialogs.settings.modelMapping.previewing')
                            : t('channels.dialogs.settings.modelMapping.previewButton')}
                        </Button>
                      </div>
                      ...
                      {mappingPreview && (
                        <div className='mt-4 space-y-3'>
                          <div className='flex flex-wrap gap-2 text-sm'>
                            <Badge>{t('channels.dialogs.settings.modelMapping.createCount', { count: mappingPreview.createCount })}</Badge>
                            <Badge variant='secondary'>
                              {t('channels.dialogs.settings.modelMapping.skipCount', { count: mappingPreview.skipCount })}
                            </Badge>
                            <Badge variant={mappingPreview.conflictCount > 0 ? 'destructive' : 'outline'}>
                              {t('channels.dialogs.settings.modelMapping.conflictCount', { count: mappingPreview.conflictCount })}
                            </Badge>
                          </div>
                          {mappingPreview.entries.length === 0 ? (
                            <p className='text-muted-foreground text-sm'>{t('channels.dialogs.settings.modelMapping.noRoutes')}</p>
                          ) : (
                            <div className='max-h-52 space-y-2 overflow-y-auto'>
                              {mappingPreview.entries.map((entry) => (
                                <div
                                  key={`${entry.from}:${entry.to}`}
                                  className='bg-background grid gap-1 rounded border p-2 text-sm sm:grid-cols-[auto_1fr]'
                                >
                                  <Badge
                                    className='w-fit'
                                    variant={
                                      entry.action === 'CONFLICT' ? 'destructive' : entry.action === 'SKIP' ? 'secondary' : 'default'
                                    }
                                  >
                                    {t(`channels.dialogs.settings.modelMapping.action.${entry.action.toLowerCase()}`)}
                                  </Badge>
                                  <div className='min-w-0'>
                                    <div className='font-mono break-all'>
                                      {entry.from} → {entry.to}
                                    </div>
                                    {entry.previousTo && entry.action === 'CONFLICT' && (
                                      <div className='text-muted-foreground break-all'>
                                        {t('channels.dialogs.settings.modelMapping.previousTarget', { target: entry.previousTo })}
                                      </div>
                                    )}
                                  </div>
                                </div>
                          ...
                          {mappingPreview.conflictCount > 0 && (
                            <label className='flex items-start gap-2 text-sm'>
                              <Checkbox checked={replaceConflicts} onCheckedChange={(checked) => setReplaceConflicts(checked === true)} />
                              <span>{t('channels.dialogs.settings.modelMapping.replaceConflicts')}</span>
                            </label>
                          )}
                          <div className='flex justify-end'>
                            <Button
                              ...
                              disabled={
                                applyMappings.isPending ||
                                mappingPreview.createCount + (replaceConflicts ? mappingPreview.conflictCount : 0) === 0
                              }
                            >
                              {applyMappings.isPending
                                ? t('channels.dialogs.settings.modelMapping.applying')
                                : t('channels.dialogs.settings.modelMapping.applyButton', {
                                    count: mappingPreview.createCount + (replaceConflicts ? mappingPreview.conflictCount : 0),
                                  })}
                            </Button>
                          </div>

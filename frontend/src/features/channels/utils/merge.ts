...
export function mergeChannelSettingsForUpdate(
  ...
  return {
    managementAdapter: pick('managementAdapter', existing?.managementAdapter ?? null),
    billingCurrency: pick('billingCurrency', existing?.billingCurrency ?? null),
    rechargeMultiplier: pick('rechargeMultiplier', existing?.rechargeMultiplier ?? null),
    ...
    autoModelMappingRules: pick('autoModelMappingRules', existing?.autoModelMappingRules ?? []),
    errorResponseRewriteRules: pick('errorResponseRewriteRules', existing?.errorResponseRewriteRules ?? []),
  };
}

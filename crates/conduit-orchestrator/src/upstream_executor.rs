...
impl Executor for UpstreamExecutor {
    ...
    async fn execute_stream_live(
        ...
        let client = self.client_for_request(request)?;
        let mut response = validate_sse_response(send(&client, request).await?).await?;
        let (tx, rx) = tokio::sync::mpsc::channel::<Result<StreamEvent, ConduitError>>(64);
